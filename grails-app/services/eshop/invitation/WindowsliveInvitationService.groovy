...
class WindowsliveInvitationService {
    ...
    def getAuthDetails(callbackUrl) {
        ...
        {
            authService = [:]
            authService.authUrl = """https://consent.live.com/Connect.aspx?wrap_client_id=${ CH.config.grails.plugin.invitation.windowslive.key as String }&wrap_callback=${ (callbackUrl as String).encodeAsURL() }&wrap_scope=WL_Contacts.View"""
            authService.callbackUrl = callbackUrl as String
            authService.key = CH.config.grails.plugin.invitation.windowslive.key as String
            authService.secret = CH.config.grails.plugin.invitation.windowslive.secret as String
        }
