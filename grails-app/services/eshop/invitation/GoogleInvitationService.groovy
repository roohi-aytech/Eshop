    ...
    def getAuthDetails(callbackUrl) {
        if (!authService) {
            authService = new ServiceBuilder().provider(GoogleApi.class)
                    .apiKey(CH.config.grails.plugin.invitation.google.key as String)
                    .apiSecret(CH.config.grails.plugin.invitation.google.secret as String)
