    ...
    <a href="<g:createLink uri="/product/${productModel?.product?.id}?model=${productModel?.id}"/>">
        ...
        <div class="attributes scrollable"
            ...
            <div class="attribute-list">
                <g:each in="${productModel?.product?.attributes?.findAll {
                    it.attributeType.showPositions.contains('productList') && it.value && it.value.toString().compareTo("N/A") != 0
                }?.sort { it.attributeType.sortIndex }}">
                ...
                <div><b><g:message code="guarantee"/>:</b> ${productModel?.product?.guaranteeList?.collect {
                    it.toString()
                }?.join('، ')}</div>
            </div>
        </div>
