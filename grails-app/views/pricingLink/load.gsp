...
<script language="javascript" type="text/javascript">
    $(document).ready(function () {
        $.ajax({
            ...
            var container = $('#linkContainer${link?.id}');
//            container.remove();
            container.html(data);
            container.find('img').removeAttr('src');
            $(window).trigger('resize');
            jQuery.ready();
//            $('body').append(container);
            %{--$('#linkContainer${link?.id}').find('img').each(function () {--}%
            %{--window.alert('salam')--}%
            %{--});--}%
            %{--$('#linkContainer${link?.id}').css('display', 'block');--}%
            %{--.--}%
            %{--each(function () {--}%
    ...
    function rel_to_abs(url) {
        ...
        if (url.toLowerCase().contains('jpg') ||
            ...
            return "${resource(dir: 'images',file: 'nothing.png')}";
        ...
        if (url.substring(0, 2) == "//")
            ...
            return location.protocol + "//${domain}" + url;
