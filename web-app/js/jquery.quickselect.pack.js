...
// active / hover:    .ui-state-hover
        ...
        return this.each(function(){
            ...
            if(input.tagName == 'INPUT'){
                ...
                var text_input = $("<input type='text' class='"+className+"' id='"+id+"_quickselect' accesskey='"+accesskey+"' tabindex='"+tabindex+"' placeholder='"+$(input).attr("placeholder")+"' />");
                if(selected_option){text_input.val($(selected_option).text());}

//                var hidden_input = $("<span style='display: none;' ></span>");
//                var hidden_input_ = $("<input type='text' id='"+id+"' name='"+input.name+"' />");
//                if($(input).attr("ng-model")){
//                    hidden_input_.attr("ng-model",$(input).attr("ng-model"))
//                    text_input.attr("ng-model",$(input).attr("ng-model").replace(".id",".label"))
//                }
//                hidden_input.append(hidden_input_)
...
//                if(selected_option){hidden_input.val(selected_option.value);}
                ...
                my_options.additionalFields = $(input);

                // Replace the select with a quickselect text_input
                $(input).after(text_input).hide(); // add text input, hidden input, remove select.
