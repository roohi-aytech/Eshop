            ...
            if (aD.is("ul") && aD.children("li").length && aD.find("img.etalage_source_image").length) {
                ...
                if (p > 1 || !c.smallthumb_hide_single) {
                    ...
                    a.each(ar, function () {
                        ...
                        a('<li><img class="etalage_small_thumb" src="' + O.replace('300x300', '50x50') + '" /></li>').data("thumb_id", G).appendTo(l)
                    });
                    ...
                    if (p > ag) {
                        ...
                        G = v.eq(p - 1).data("id");
                        a('<li class="etalage_smallthumb_first"><img class="etalage_small_thumb" src="' + O + '" /></li>').data("src", O).data("thumb_id", G).css({opacity: c.smallthumb_inactive_opacity}).prependTo(l);
                        O = ar.eq(0).data("src");
                        G = v.eq(0).data("id");
                        a('<li><img class="etalage_small_thumb" src="' + O + '" /></li>').data("src", O).data("thumb_id", G).css({opacity: c.smallthumb_inactive_opacity}).appendTo(l);
                        aR = l.children("li");
                ...
                var d = parseInt(I.css("borderTopWidth"), 10), ax = parseInt(c.zoom_area_distance, 10), H = parseInt(I.css("paddingTop"), 10), R, aW;
                ...
                at = {margin: 0, padding: 0, left: (o - aj) / 2, top: (K - aH) / 2};
                if (c.smallthumbs_position === "left") {
                    at.left = "+=" + n + N
                } else {
                    if (c.smallthumbs_position === "top") {
