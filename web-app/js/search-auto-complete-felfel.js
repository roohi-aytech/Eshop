...
function setupSearchAutoComplete(form) {
    ...
    dropDown.width($(form).width() - 10);
    dropDown.css('top', searchInput.offset().top + searchInput.height() + 10);
    dropDown.css('left', $(form).offset().left - 1);
...
function showAutoCompleteResult(response, parameters) {
    if (response != '') {
        ...
        lastAutoCompleteDropDown.find('.search-auto-complete-drop-down-inner').rollbar({zIndex: 100, wheelSpeed: 10});
