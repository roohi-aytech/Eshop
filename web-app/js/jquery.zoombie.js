...
(function ($) {
    $.fn.zoombieLens = function (options) {
        ...
        return this.each(function () {
            ...
            var imageTag = "<img style='display:none;' src='" + imageSrc.replace('500x500', 'max') + "' />";

            var widthRatio = 0;
            var heightRatio = 0;

            $(imageTag).load(function () {
                obj.css('max-width', $(this).width())
                widthRatio = $(this).width() / obj.width();
                heightRatio = $(this).height() / obj.height();
            }).appendTo($(this).parent());

            target.css({ backgroundImage: "url('" + imageSrc.replace('500x500', 'max') + "')" });
...
(function ($) {
	...
	$.fn.zoombie = function (options) {
		return this.each(function () {
			...
			if (!settings.url) {
				settings.url = $(source).find('img').attr('src').replace('500x500', 'max');
				if (!settings.url) {
					return;
				}
			}

//            var imageTag = "<img style='display:none;' src='" + settings.url + "' />";
//            var obj = $(this);
//            $(imageTag).load(function () {
//                obj.css('max-width', $(this).width())
//            });
